...
function CoverLetterEditor({ coverLetterId, coverLetterTitle, header, content, setCoverLetterTitle, setHeader, setContent }) {
    const [user, loading, error] = useAuthState(auth);
    const [editorState, setEditorState] = useState(() => {
      try {
         // Convert from raw content if available
         return content ? EditorState.createWithContent(convertFromRaw(content)) : EditorState.createEmpty();
      } catch (error) {
